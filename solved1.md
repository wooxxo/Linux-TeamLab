...
#### a. 로그 파일에서 로그인 실패한 줄만 출력하라
```
grep "POST /api/login" webserver_log.log | grep " 401 "
```
#### b. 유저명 / 실패횟수 / 마지막시도시간 / IP를 추출하여 `result.csv` 로 저장하라
```
...
```
#### c. `result.csv` 에서 실패 10회 이상인 유저와 IP를 `blacklist.txt` 로 추출하라
```bash
...
**출력 예시**
```text
[10/Jan/2024:00:01:34 +0900] 185.220.101.5 → GET /api/orders (200, 2.117s)
```
...
- 시간 정보가 `[10/Jan/2024:00:01:34 +0900]` 형태로 출력되어야 한다.
- IP, METHOD, URL, 상태코드, 응답시간이 순서대로 출력되어야 한다.
- 큰따옴표(`"`)와 대괄호(`[]`)가 제거되거나 정리된 형태여야 한다.
- 원본 로그 파일은 변경되지 않아야 한다.

#### c. 실행 결과

![문제4풀이](Images/problem4.png)
...
![권한설정결과](https://github.com/user-attachments/assets/5bdcfc95-41eb-4a36-8901-8e22886d9652)
