...
![아카이빙결과1](https://github.com/user-attachments/assets/3ed12b84-6066-40e0-afe9-d1ea92c4ef98)

![아카이빙결과2](https://github.com/user-attachments/assets/7c48b4ba-11f4-42af-98b6-17289b1747cc)
...
- JSON 파일 형태로 만들어진 로그 파일을 활용해 `jq`로 더욱 간단하게 스크립트를 작성해 본다.
...
4. **`.[$lvl] += 1` (카운팅 로직)**
   ...
   - 예: 처음 `WARN`을 만나면 `{"WARN": 1}`이 되고, 다음에 또 `WARN`을 만나면 `{"WARN": 2}`로 누적
...
SRE(사이트 신뢰성 엔지니어) 또는 백엔드 개발자로서 서버 성능 병목을 파악하기 위해, **응답 시간이 4초 이상 소요된 무거운 요청들의 엔드포인트(API URL)만 추출**해야 한다.
...
- **`.path`**
  - 통과된 지연 로그 객체에서 요청 경로(`.path`) 값만 추출
- **`-r` (raw-output 옵션)**
